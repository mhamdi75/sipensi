...
class PenilaianController extends BaseController
{
    ...
    {
        $guruMapel = $this->guruMapel->find($id);
        if ($guruMapel) {
            $guruMapel = $this->db->table('guru_mapel gm')->select('gm.id, m.nama as nama_mapel')->join('mapel m', 'gm.mapel_id = m.id')->where('gm.id', $id)->get()->getRow();
            $kirim['guruMapel'] = $guruMapel;
            $kirim['kelas'] = $this->db->table('guru_mapel_detail gmd')->select('k.*,gmd.id as gmd_id')->join('kelas k', 'gmd.kelas_id = k.id')->where('gmd.guru_mapel_id', $guruMapel->id)->get()->getResult();
            $kirim['tahunAjaran'] = $this->tahunAjaran->where(['status' => 1])->first();
            return view('guru/penilaian/index', $kirim);
        } else {
    ...
    public function loadView()
    {
        if ($this->request->getPost()) {
            ...
            if ($validation) {
                ...
                $kirim['gmd'] = $this->request->getPost('gmd');
                ...
                $view = view('guru/penilaian/data', $kirim);
    ...
    public function cetak($tahun_ajaran, $gmd, $kelas)
    {
        $guruMapelDetail = $this->db->table('guru_mapel_detail')->where('id', $gmd)->get()->getRow();
        $kirim['guruMapel'] = $this->db->table('guru_mapel gm')->select('gm.id, m.nama as nama_mapel')->join('mapel m', 'gm.mapel_id = m.id')->where('gm.id', $guruMapelDetail->guru_mapel_id)->get()->getRow();
        $kirim['db'] = $this->db;
        $kirim['tahun_ajaran'] = $tahun_ajaran;
        $kirim['tahunAjaran'] = $this->tahunAjaran->find($tahun_ajaran);
        $kirim['gmd'] = $gmd;
        $kirim['kelas'] = $this->kelas->find($kelas);
        $kirim['kategoriNilai'] = $this->kategoriNilai->findAll();
        $kirim['siswa'] = $this->db->table('tahun_ajaran_detail tad')->select('tad.id as tad_id,s.*')->join('siswa s', 'tad.siswa_id = s.id')->where([
            'tad.tahun_ajaran_id' => $tahun_ajaran,
            'tad.kelas_id' => $kelas,
        ])->orderBy('s.nis', 'asc')->get()->getResult();
        return view('guru/penilaian/cetak', $kirim);
    }
}
