               ...
               <tr>
                    ...
                    ?>
                         ...
                         <?php } else {
                              $statusForm = true;
                         ?>
                              <td class="text-end"><input type="number" required class="form-control form-control-sm inputanNilai" data-ks="<?= $k->id ?>" data-tad="<?= $s->tad_id ?>" value="<?= isset($dataNilai) ? $dataNilai->nilai : '' ?>" placeholder="Nilai"></td>
                         <?php } ?>
                    <?php endforeach;
...
<script>
     $(".inputanNilai").change(function() {
          ...
          if (ks && tad) {
               $.ajax({
                    url: "<?= base_url('guru-bk/penilaian-sikap/aksi') ?>",
                    type: "POST",
                    data: {
                         ks: ks,
                         tad: tad,
     ...
     $("#formPenilaian").submit(function() {
          $.ajax({
               url: "<?= base_url('guru-bk/penilaian-sikap/selesai') ?>",
               type: "POST",
               data: {
